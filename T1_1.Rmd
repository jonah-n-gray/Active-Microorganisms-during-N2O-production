...
for (i in 1:(length(t)-1)) {
  ...
  pre=mean(dat$N2O[(N2O_max_index[x]-(q+3)):(N2O_max_index[x]-3)]) #we are excluding the 3 seconds before the max
...
write.csv(sum_sampledat, file="T1_1_final")
plot(t,N2O_ppb)
hist(N2O_ppb)
boxplot(N2O_ppb)
boxplot(mg_N2O_N)
